...
def _load_json_list(file_path: str) -> list[dict]:
    try:
        ...
        if not isinstance(data, list):
            print(
                f"Error: {file_path} must contain a list.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if not all(isinstance(x, dict) for x in data):
            print(
                f"Error: {file_path} must contain a list of objects.",
                file=sys.stderr,
            )
            sys.exit(1)
...
def load_prompts(file_path: str) -> list[PromptInput]:
    ...
    for i, item in enumerate(raw_data):
        try:
            ...
            print(
                f"[SKIP] invalid prompt at index {i}:\n{e}",
                file=sys.stderr,
            )
...
def load_definitions(file_path: str) -> list[FunctionDefinition]:
    ...
    for i, item in enumerate(raw_data):
        try:
            ...
            print(
                f"[SKIP] invalid function at index {i}:\n{e}",
                file=sys.stderr,
            )
